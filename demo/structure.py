...
models = [Autoencoder(input_size, latent_size) for _ in range(8)]
...
def get_minipos(z):
    BOUND_L, BOUND_R, BOUND_U, BOUND_D = bounds[model_idx]
...
while True:
    ...
    state = torch.tensor([GOAL_X, GOAL_Y, 1], dtype=torch.float32)
    z = generate_latent_representation(state, model)
    mx, my = get_minipos(z)
    ...
    # Draw the ball
    pygame.draw.circle(screen, GREEN if cstate == 0 else BLUE, (ball_x * GRID_SIZE + GRID_SIZE // 2, ball_y * GRID_SIZE + GRID_SIZE // 2), GRID_SIZE // 2)
