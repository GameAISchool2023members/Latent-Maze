...
input_size = 2#GRID_WIDTH * GRID_HEIGHT
latent_size = 2
...
def generate_random_state():
    return torch.rand(2)#(GRID_HEIGHT, GRID_WIDTH))
...
while True:
    ...
    state = torch.tensor([GOAL_X, GOAL_Y], dtype=torch.float32)
    z = generate_latent_representation(state)
    mx, my = get_minipos(z)
    ...
    state = torch.tensor([ball_x, ball_y], dtype=torch.float32)
    z = generate_latent_representation(state)
    mx, my = get_minipos(z)
